      ...
      >
        ...
        <el-table-column prop="sex" label="性别" width="70" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.sex === 'FEMALE'" style="color: #f84a4a"
              >女</span
            >
            <span v-else-if="scope.row.sex === 'MALE'" style="color: #20a0ff"
              >男</span
            >
            <span v-else>未知</span>
          </template>
        ...
        <el-table-column label="操作" width="210" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              type="success"
              size="mini"
              @click="openUpdatePasswordDialog(scope.$index)"
              >改密</el-button
            >
            <el-popconfirm
    ...
    <div>
      ...
      >
        <el-input
          ...
          size="small"
          autocomplete="off"
          placeholder="请输入账户新密码"
        />
        <span slot="footer">
          <el-button type="primary" size="small" @click="updatePassword"
            >更 新</el-button
          >
          <el-button size="small" @click="updatePasswordDialogVisible = false"
            >取 消</el-button
          >
        </span>
      </el-dialog>
      ...
      >
        <div style="color: red; margin-bottom: 20px">
          * 新账户默认密码为<strong>654321</strong>，创建账户后请修改
        </div>
...
export default {
  ...
  methods: {
    ...
    openEnableDialogVisible(index) {
      ...
      this.enableDialogTitle = `${user.enable ? "启用" : "禁用"}学员[${
        user.nickname
      }]`;
    ...
    disenablle() {
      ...
      fun(user.id).then((resp) => {
        const msg =
          "学员账户[" + user.nickname + "]已" + (user.enable ? "启用" : "禁用");
        if (user.enable) {
